...
const respostaAllMenus = fetch("../JSONs/datas.json").then(respostaData => {
    ...
    var countJson = Object.keys(jsonData.objMenuJson).length
...
//JSON
    for(var i = 0 ; i < countJson ; i++){
        var obj = jsonData.objMenuJson[i];
        var itensMenu = "<li><a href="+obj.url+" "+obj.attribute+">"+obj.title+"</a></li>";

        document.getElementById('AndroidMenu').innerHTML +=itensMenu;
    }
...
(function Cronometro(){
    ...
    function timerCrono(){
        ...
        let formatTime = (hh < 10 ? '0' + hh : hh) + ':' + (mm < 10 ? '0' + mm : mm) + ':' + (ss < 10 ? '0' + ss : ss) + ' - THIAGONÇALVES'

        document.querySelector('head title').innerHTML = formatTime
    }
    

    document.querySelector('.contadorTempo .Iniciar').addEventListener('click', ()=>{
    ...
    document.querySelector('.contadorTempo .Parar').addEventListener('click', ()=>{
        ...
        document.querySelector('head title').innerHTML = 'THIAGONÇALVES'
    })
})()